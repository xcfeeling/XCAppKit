//
//  HsKeyChainItemWrapper.m
//  HsAppKit
//
//  Created by 陈品 on 2024/5/8.
//

#import "HsKeyChainItemWrapper.h"

NSString * const kHsService = @"com.hsdata.analytics.udid";
NSString * const kHsUdidAccount = @"com.hsdata.analytics.udid";

@implementation HsKeyChainItemWrapper
+ (NSString *)hsUdid {
    NSDictionary *result = [self fetchPasswordWithAccount:kHsUdidAccount service:kHsService];
    NSString *udid = [result objectForKey:(__bridge id)kSecValueData];
    return udid;
}

+ (NSString *)saveUdid:(NSString *)udid {
    BOOL sucess = [self saveOrUpdatePassword:udid account:kHsUdidAccount service:kHsService];
