...
//  Common
//
//  Created by xucheng on 2023/3/8.
//
...
public extension UIImage {
    ...
    ///   - imageSize: 图片的大小
    /// - Returns: 剪切后的图片
    public func isRoundCorner(radius: CGFloat = 3, byRoundingCorners corners: UIRectCorner = .allCorners, imageSize: CGSize?) -> UIImage? {
    ...
    /// 设置圆形图片
    /// - Returns: 圆形图片
    public func isCircleImage() -> UIImage? {
    ...
    public static func image(from layer: CALayer, scale: CGFloat = 0.0) -> UIImage? {
    ...
    /// alpha: 透明度
    /// - Returns: newImage
    public func imageByApplayingAlpha(_ alpha: CGFloat) -> UIImage {
    ...
    /// - Returns: 返回水印图片
    public func drawTextInImage(drawTextframe: CGRect, drawText: String, withAttributes: [NSAttributedString.Key : Any]? = nil) -> UIImage {
        // 开启图片上下文
    ...
    /// - Returns: 返回图片
    public static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        // 过滤空内容
        ...
        let textsize = text.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: UIScreen.main.bounds.width, height: CGFloat(MAXFLOAT)))
    ...
    ///   - blendMode: 模式
    /// - Returns: 返回更改后的图片颜色
    public func tint(color: UIColor, blendMode: CGBlendMode = .destinationIn) -> UIImage? {
        /**
    ...
    // MARK: 1.12、保存图片到相册
    /// 保存图片到相册
    public func saveImageToPhotoAlbum(_ result: ((Bool)->())?) {
        self.saveToPhotoAlbum(result)
    }
    
    // MARK: 1.13、保存图片到相册(建议使用这个)
    /// 保存图片到相册
    public func savePhotosImageToAlbum(completion: @escaping ((Bool, Error?) -> Void)) {
    ...
    /// - Returns: 返回模糊后的图片
    public func getGaussianBlurImage(fuzzyValue: CGFloat = 20) -> UIImage? {
        // 生成的高斯模糊滤镜图片
    ...
    /// - Parameter fuzzyValue: 设置模糊半径值（越大越模糊）
    /// - Returns: 返回像素化后的图片
    public func getPixellateImage(fuzzyValue: CGFloat = 20) -> UIImage? {
        // 生成的高斯模糊滤镜图片
...
public extension UIImage {
    ...
    ///   - round: 圆角大小
    /// - Returns: 返回对应的图片
    public static func image(color: UIColor, size: CGSize, corners: UIRectCorner, radius: CGFloat) -> UIImage? {
    ...
    // MARK: 2.3、生成渐变色的图片 [0xB0E0E6, 0x00CED1, 0x2E8B57]
    /// 生成渐变色的图片[0xB0E0E6, 0x00CED1, 0x2E8B57]
    /// - Parameters:
    ///   - hexs: 十六进制数组
    ///   - size: 图片大小
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    public static func gradient(_ hexs: [Int], size: CGSize = CGSize(width: 1, height: 1), locations:[CGFloat]? = nil, direction: ImageGradientDirection = .horizontal) -> UIImage? {
        return gradient(hexs.map{ UIColor(hex: $0) }, size: size, locations: locations, direction: direction)
    }
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    public static func gradient(_ colors: [UIColor], size: CGSize = CGSize(width: 10, height: 10), locations:[CGFloat]? = nil, direction: ImageGradientDirection = .horizontal) -> UIImage? {
...
public extension UIImage {
    ...
    /// - Parameter size: 图片尺寸
    /// - Returns: 固定大小的 image
    public func solidTo(size: CGSize) -> UIImage? {
        let w = size.width
        let h = size.height
    ...
    /// - Returns: 等比缩放 后的图片
    public func scaleTo(scale: CGFloat) -> UIImage? {
    ...
    /// 图片中间1*1拉伸——如气泡一般
    /// - Returns: 拉伸后的图片
    public func strechAsBubble() -> UIImage {
    ...
    ///   - resizingMode: 拉伸模式
    /// - Returns: 返回拉伸后的图片
    public func strechBubble(edgeInsets: UIEdgeInsets, resizingMode: UIImage.ResizingMode = .stretch) -> UIImage {
        // 拉伸
...
public extension UIImage {
    ...
    /// - Parameter mode: 压缩模式
    /// - Returns: 压缩后Data
    public func compress(mode: CompressionMode = .medium) -> Data? {
    ...
    public func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
    ...
    /// - Returns: 压缩后数据
    public func compressDataSize(maxSize: Int = 1024 * 1024 * 2) -> Data? {
    ...
    /// - Returns: 调整后图片
    public func resizeIO(resizeSize: CGSize) -> UIImage? {
        if size == resizeSize {
            return self
        }
    ...
    /// - Parameter resizeSize: 图片调整Size
    /// - Returns: 调整后图片
    public func resizeCG(resizeSize: CGSize) -> UIImage? {
        if size == resizeSize {
            return self
        }
...
public extension UIImage {
    ...
    ///   - logoImage: logo图片
    /// - Returns: 返回生成二维码图片
    public static func QRImage(with content: String, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
    ...
    // MARK: 5.2、获取图片中二维码数组
    public func getImageQRImage() -> [CIQRCodeFeature] {
        let qrcodeImg = self
        let context = CIContext(options: nil)
    ...
    public func getImageQRImageInfo() -> [String] {
...
public extension UIImage {
    ...
    public static func checkImageDataType(data: Data?) -> DataType {
        guard data != nil else {
            return .defaultType
        }
        let c = data![0]
    ...
    public static func gif(url: String) -> UIImage? {
        // Validate URL
    ...
    @available(iOS 9.0, *)
    public static func gif(asset: String) -> UIImage? {
    ...
    public static func gifImages(asset: String) -> (gifImages: [UIImage]?, duration: TimeInterval?) {
        // Create source from assets catalog
        guard let dataAsset = NSDataAsset(name: asset) else {
            debugPrint("SwiftGif: Cannot turn image named \"\(asset)\" into NSDataAsset")
            return (nil, nil)
    ...
    /// - Parameter name:图片的名字
    /// - Returns: 分解后的图片和gif时间
    public static func gifImages(name: String) -> (gifImages: [UIImage]?, duration: TimeInterval?) {
        // Check for existance of gif
        guard let bundleURL = Bundle.main
                .url(forResource: name, withExtension: "gif") else {
                    debugPrint("SwiftGif: This image named \"\(name)\" does not exist")
    ...
    /// - Parameter url: gif图片的网络地址
    /// - Returns: 分解后的图片和gif时间
    public static func gifImages(url: String) -> (gifImages: [UIImage]?, duration: TimeInterval?) {
        // Validate URL
        guard let bundleURL = URL(string: url) else {
            debugPrint("SwiftGif: This image named \"\(url)\" does not exist")
            return (nil, nil)
