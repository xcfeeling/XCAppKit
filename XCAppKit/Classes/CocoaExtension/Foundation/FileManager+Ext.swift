...
public extension FileManager {
    ...
    public static func DocumnetsDirectory() -> String {
    ...
    public static func PreferencesDirectory() -> String {
...
public extension FileManager {
    ...
    @discardableResult
    public static func createFolder(folderPath: String) -> (isSuccess: Bool, error: String) {
    ...
    @discardableResult
    public static func removefolder(folderPath: String) -> (isSuccess: Bool, error: String) {
    ...
    @discardableResult
    public static func createFile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径才会创建
    ...
    /// - Parameter filePath: 文件路径
    @discardableResult
    public static func removefile(filePath: String) -> (isSuccess: Bool, error: String) {
        guard judgeFileOrFolderExists(filePath: filePath) else {
            // 不存在的文件路径就不需要要移除
            return (true, "")
        }
    ...
    /// - Returns: 写入的结果
    @discardableResult
    public static func writeToFile(writeType: FileWriteType, content: Any, writePath: String) -> (isSuccess: Bool, error: String) {
    ...
    /// - Returns: 返回读取的内容
    @discardableResult
    public static func readFromFile(readType: FileWriteType, readPath: String) -> (isSuccess: Bool, content: Any?, error: String) {
    ...
    /// - Returns: 拷贝的结果
    @discardableResult
    public static func copyFile(type: MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
    ...
    ///   - toFile: 移动后的文件路径
    @discardableResult
    public static func moveFile(type: MoveOrCopyType, fromeFilePath: String, toFilePath: String, isOverwrite: Bool = true) -> (isSuccess: Bool, error: String) {
        // 1、先判断被拷贝路径是否存在
        guard judgeFileOrFolderExists(filePath: fromeFilePath) else {
    ...
    /// 判断文件或文件夹是否存在
    public static func judgeFileOrFolderExists(filePath: String) -> Bool {
    ...
    public static func directoryAtPath(path: String) -> String {
        return (path as NSString).deletingLastPathComponent
    }
    
    // MARK: 2.12、判断目录是否可读
    public static func judegeIsReadableFile(path: String) -> Bool {
        return fileManager.isReadableFile(atPath: path)
    }
    
    // MARK: 2.13、判断目录是否可写
    public static func judegeIsWritableFile(path: String) -> Bool {
        return fileManager.isReadableFile(atPath: path)
    }
    
    // MARK: 2.14、判断目录是否可执行
    public static func judegeIsExecutableFile(path: String) -> Bool {
        return fileManager.isExecutableFile(atPath: path)
    }
    
    // MARK: 2.15、判断目录是否可删除
    public static func judegeIsDeletableFile(path: String) -> Bool {
    ...
    /// - Parameter path: 文件路径
    /// - Returns: 文件扩展类型
    public static func fileSuffixAtPath(path: String) -> String {
    ...
    ///   - suffix: 是否需要后缀，默认需要
    /// - Returns: 文件名称
    public static func fileName(path: String, suffix: Bool = true) -> String {
    ...
    public static func shallowSearchAllFiles(folderPath: String) -> Array<String>? {
    ...
    public static func getAllFileNames(folderPath: String) -> Array<String>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
    ...
    public static func deepSearchAllFiles(folderPath: String) -> Array<Any>? {
        // 查看文件夹是否存在，如果存在就直接读取，不存在就直接反空
    ...
    /// - Returns: 单个文件或文件夹的大小
    public static func fileOrDirectorySingleSize(filePath: String) -> UInt64 {
        // 1、先判断文件路径是否存在
        guard judgeFileOrFolderExists(filePath: filePath) else {
            return 0
        }
    ...
    /// - Parameter path: (文件夹/文件) 的路径
    /// - Returns: (文件夹/文件) 的大小
    public static func fileOrDirectorySize(path: String) -> String {
